...
\begin{center}
\centering
\includegraphics[width=0.64\textwidth]{fig_dashboard_metrics.png}
\vspace{2pt}
...
Landmark symmetry in a C4-symmetric arena degraded the pRNN's spatial representation along three measurable dimensions: decoding error increased from 0.365 to 0.524, metric geometry shifted toward city-block structure (DTG = $-0.052$ vs $-0.020$), and unit-level rotational aliasing increased 2.7-fold (RA = 0.187 vs 0.069, shuffle z-score = $\mathtt{[SHUFFLE:S4]}$). Global orientational degeneracy did not occur: cross-seed RSA alignment exceeded 0.97 in all conditions and PAA gain remained below 0.01. The C2 arena produced a qualitatively distinct result: positive C2 Contrast (+0.045) indicated that the population code specifically encoded the C2 equivalence relation of the observation function, a structure too selective to be explained by generic precision loss.
