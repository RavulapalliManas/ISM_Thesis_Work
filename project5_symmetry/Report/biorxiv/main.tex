...
does. Read through its own $C_4$ quotient, the $C_4$ arena approaches the
group-theoretic ceiling: a $G$-folded code cannot exceed $1/|G|$ phase accuracy, and the
invariant encodings fall to within a few percent of it ($0.482$ against a predicted $0.500$ for
a $C_2$-invariant encoding; $0.275$ against $0.250$ for the only $C_4$-invariant one). Finally,
the entire effect is contingent on the objective:
...
($p = 0.59$). One step of prediction restores it. The same mechanism reproduces the place-field
repetition seen across visually identical compartments in rodents \citep{grieves2016}, folding
translationally related rooms and lifting rotationally related ones, and it survives a
head-direction signal corrupted on a third of its steps. We conclude that a self-motion signal resolves only those symmetries under which it is
...
(Fig.~\ref{fig:placecells}). In the asymmetric arena under the full compass, about $480$ of the
...
folds the $C_2$ arena to near chance (Table~\ref{tab:phase}).
