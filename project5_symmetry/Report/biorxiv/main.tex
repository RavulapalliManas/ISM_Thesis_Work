...
($p = 0.59$). One step of prediction restores it completely. The mechanism has external
validity and is robust: it reproduces the place-field repetition seen across visually identical
compartments in rodents \citep{grieves2016}, folding translationally related rooms and lifting
rotationally related ones, and it survives a head-direction signal corrupted on a third of its
steps. We conclude that a self-motion signal resolves only those symmetries under which it is
not itself invariant, and only when the objective forces the network to integrate it. Along the
way we show that two natural readouts, rotational autocorrelation and odd isotypic power, are
respectively blind to and confounded by this phenomenon.
\end{abstract}
...
networks hit exactly. The effect is a property of \emph{predictive} learning, since a matched
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_encodings.pdf}
\caption{The four head-direction encodings, as $4\times4$ transforms of the one-hot heading
block $\{$E, S, W, N$\}$. \textit{full} is the identity (two bits); \textit{parity} and
\textit{axis} each collapse the compass to one bit but partition the headings differently;
\textit{const} discards it entirely. \textit{axis} and \textit{const} are invariant under the
$180^\circ$ rotation and so fold the $C_2$ arena, while \textit{const} is the only encoding
invariant under the full $C_4$ group. \textit{axis} and \textit{parity} carry identical
information and differ only in this invariance.}
\label{fig:encodings}
\end{figure}

The two hypotheses make opposite predictions. If folding tracks how much information the
compass carries, \textit{axis} and \textit{parity} behave alike (Fig.~\ref{fig:encodings}). If
...
no topology-before-geometry claim. All results use a single architecture (hidden size $500$,
