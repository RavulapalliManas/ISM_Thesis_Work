...
hippocampus --- and, in a published CA1 dataset \citep{hockeimer2023}, the repeating fields of a
cell share a directional preference exactly across the translation-related alleys the compass
cannot separate, the fold measured directly in the brain. Throughout we distinguish a folded code
from a broken one; the folded networks build a reproducible spatial map and merely discard which
copy of the fundamental domain the animal occupies.
...
\subsection{A learned, unanchored compass folds symmetric arenas on its own}
...
network decodes orbit phase almost perfectly ($0.969$, $n=2$), because the arena's unique
landmarks anchor absolute position. In the $C_2$ arena it falls to chance ($0.531$, chance
...
($0.525$, $n=8$; $R^2_{\text{domain}} = -0.47$, the four-fold signature a $C_2$ decoder cannot
see). This is the same fold the hand-designed \textit{axis} and \textit{const} encodings produce,
now arising from the compass itself. The fold is therefore not an artefact of imposing an
...
every view identical across the four rooms at matched local position and heading. A predictive
...
stayed fully intact ($R^2 = 0.96$). The two-room dissociation is thus not a small-$n$ artefact: at
...
Finally, the prediction holds in real neurons. We reanalysed the CA1 recordings of
\citet{hockeimer2023}, in which rats foraged a city-block maze whose alleys run in two
orthogonal orientations, so that alleys of the same orientation are related by translation (the
compass reads the same) and alleys of different orientation by a $90^\circ$ rotation (the compass
differs). Our account predicts that a cell's repeating fields should share a directional
preference across same-orientation alleys, where head direction cannot separate them, and not
across rotated ones. They do: within a repeating cell, the directional index of same-orientation
field pairs is positively correlated ($r = 0.23$, shuffle-null $p = 0.007$; mixed-effects
intraclass correlation $0.31$ with rat and cell as random effects, $54$ repeating cells, $5$
rats), whereas fields in orthogonally-oriented alleys lie on incomparable travel axes. This is
the fold, measured in the hippocampus: repeated fields inherit the same directional tuning
exactly where the self-motion signal is invariant under the transformation relating them.
...
head-direction signal is supplied rather than continuous: a control corrupting the compass on
$30\%$ of steps leaves the dissociation intact, and a \emph{learned} compass integrated from
angular velocity alone reproduces the fold on its own (Results), but a continuous compass that
drifts and is re-anchored by vision over time is not modelled here.
